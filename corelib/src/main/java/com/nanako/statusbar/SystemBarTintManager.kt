...
import com.nanako.statusbar.SystemBarTintManager.SystemBarConfig
import com.nanako.statusbar.SystemBarTintManager
...
class SystemBarTintManager @TargetApi(19) constructor(activity: Activity) {
    ...
    class SystemBarConfig constructor(
        activity: Activity,
        translucentStatusBar: Boolean, traslucentNavBar: Boolean
    ) {
        ...
        fun getPixelInsetTop(withActionBar: Boolean): Int {
            return ((if (mTranslucentStatusBar) statusBarHeight else 0)
                    + if (withActionBar) actionBarHeight else 0)
        }
